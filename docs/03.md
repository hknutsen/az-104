...
- Three types of SAS:
  1. User delegation SAS
    ...
    - Only supported for Blob service
  ...
  1. Service SAS
    ...
    - Delegates access to a resource in any one of four Azure Storage services: Blob, Queue, Table, or File
  1. Account SAS
    - Secured using a storage account key
    - Same controls as a service SAS, but can also control access to service-level operations such as `Get Service Stats`
